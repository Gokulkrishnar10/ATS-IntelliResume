...
def get_groq_client():
    ...
    # Try to get from Streamlit secrets (if secrets.toml exists)
    try:
        if "GROQ_API_KEY" in st.secrets:
            api_key = st.secrets["GROQ_API_KEY"]
    except:
        pass  # Secrets file doesn't exist, continue to next option
...
# ==========================================
def run_phase_9_export(optimized_resume, resume_struct, jd_data, scoring_data, qa_report):
    """
    ...
    if linkedin_url:
        p = doc.add_paragraph(f"LinkedIn: {linkedin_url}")
        p.alignment = WD_ALIGN_PARAGRAPH.LEFT
        p.paragraph_format.line_spacing = 1.15
        p.paragraph_format.space_after = Pt(2)
        for run in p.runs:
    ...
    # GitHub (if available)
    if github_url:
        p = doc.add_paragraph(f"GitHub: {github_url}")
        p.alignment = WD_ALIGN_PARAGRAPH.LEFT
        p.paragraph_format.line_spacing = 1.15
    ...
    if PDF_AVAILABLE:
        ...
        if github_url:
            story.append(Paragraph(f"GitHub: {github_url}", contact_style))
    ...
    log_system(f"📁 Output Directory: {os.path.abspath(OUTPUT_DIR)}/")
...
# ==========================================
def run_full_pipeline(uploaded_file, jd_text, api_key, guidance_mode="A"):
    """
    ...
    try:
        ...
        log_system("="*80)
        export_results = run_phase_9_export(optimized_resume, resume_struct, jd_data, scoring_results, qa_results)
        ...
        with progress_container:
            ...
            try:
                ...
                for i, (progress, text) in enumerate(phases):
                    ...
                    if i == 0:
                        results = run_full_pipeline(uploaded_resume, jd_text_input, INTERNAL_GROQ_API_KEY, guidance_mode)
...
if 'results' in st.session_state and st.session_state['results']:
    ...
    features = [
        ...
        "🛣️ **Career Guidance** - Personalized learning paths and project suggestions",
